...
from backend.models import ErrorLog, MoneyLog
from backend.settings import UNBELIEVABOAT_BOT_ID, WORK_CHANNEL_ID
...
class BotTools(DiscordAndSearch, Extractor):
    ...
    def collect_collect_daily(self):
        ...
        if success_coll and success_bal:
            ...
            error = ErrorLog(owner=self.bot, comment="Error while collecting daily", body=error_body)
            error.save()
    ...
    def save_result(self, success, error_body, operation):
        if success:
            ...
            sleep(3)
            message = self.get_latest_money_bot_message(WORK_CHANNEL_ID, UNBELIEVABOAT_BOT_ID, self.bot.name)
            if message:
                ...
                if operation == "work":
                    ...
                    error = ErrorLog(
                        owner=self.bot, comment=f"No money found in message, operation: {operation}", body=message
                    )
                    error.save()
            else:
                logger.info("No message found")
                error = ErrorLog(owner=self.bot, comment="No message found")
                error.save()
        else:
            error = ErrorLog(owner=self.bot, comment=f"Error while completing {operation}", body=error_body)
            error.save()
